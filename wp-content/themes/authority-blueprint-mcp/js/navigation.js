document.addEventListener("DOMContentLoaded", function () {
  ...
  if (reoonForm) {
    reoonForm.addEventListener("submit", async function (e) {
      ...
      const resultDiv = reoonForm.parentElement.querySelector(
        ".reoon-email-result",
      );
      if (!email) {
        showErrorSummary(reoonForm, [
          { fieldId: "reoon_email", message: "Email is required." },
        ]);
  ...
  if (acumbaForm) {
    acumbaForm.addEventListener("submit", async function (e) {
      ...
      const resultDiv = acumbaForm.parentElement.querySelector(
        ".acumbamail-newsletter-result",
      );
      if (!email) {
        showErrorSummary(acumbaForm, [
          { fieldId: "acumbamail_email", message: "Email is required." },
        ]);
  ...
  if (certopusForm) {
    certopusForm.addEventListener("submit", async function (e) {
      ...
      const resultDiv = certopusForm.parentElement.querySelector(
        ".certopus-certificate-result",
      );
      const errors = [];
      if (!name)
        errors.push({ fieldId: "certopus_name", message: "Name is required." });
      if (!email)
        errors.push({
          fieldId: "certopus_email",
...
function showErrorSummary(form, errors) {
  ...
  summary.innerHTML =
    "<strong>There were errors with your submission:</strong><ul>" +
    errors
