...
(function (global, factory) {
  ...
  LibsBundle.prototype.addLib = function (libName, options) {
    var libSpec =
      libName instanceof LibSpec ? libName : new LibSpec(libName, options);
  ...
  LibsBundle.prototype.getLibVersion = function (libName, versionName) {
    ...
    console.warn(
      'Could not find requested library "' +
        libName +
        '" and version "' +
        versionName +
        '"',
   ...
   */
  LibsBundle.prototype.makeLibVersionsRadios = function (
    libName,
    versionsNames,
    fragment,
  ) {
   ...
   * @param fragment? <Node|DocumentFragment>
   */
  LibsBundle.prototype.insertLibVersionsRadioHere = function (
    libName,
    versionsNames,
    fragment,
  ) {
   ...
   */
  LibsBundle.prototype.getAndSelectVersionsAssetsList = function (
    libVersionsDict,
  ) {
    var libSpecs = this.libs,
      lib_i = 0,
  ...
  LibSpec.prototype.addVersion = function (versionName, options) {
    var versionSpec =
      versionName instanceof VersionSpec
        ? versionName
  ...
  LibSpec.prototype.getVersionFallback = function (versionName) {
    ...
    if (typeof versionName !== "string" || versionName === "(none)") {
      ...
      if (this.mandatory) {
        console.warn(
          'No version requested for mandatory library "' + this.name + '".',
        );
    ...
    if (libVersionSpec) {
      ...
      console.warn(
        'Requested library "' +
          ...
          '" with version "' +
          versionName +
          '", but not found in configuration.',
      );
      return this.getDefaultVersion();
    }
  };
  ...
  LibSpec.prototype.getDefaultVersion = function () {
    ...
    if (libDefaultVersionSpec) {
      console.log(
        'Using default configuration (version: "' +
          libDefaultVersionSpec.name +
          '").',
      );
  ...
  VersionSpec.prototype.addAsset = function (assetSpec) {
    var assetSpec =
      assetSpec instanceof AssetSpec ? assetSpec : new AssetSpec(assetSpec);
   ...
   * @returns {Promise}
   */
  VersionSpec.prototype.checkAssetsAvailability = function (
    automaticallyEnableOrDisable,
  ) {
    var checks = [],
      self = this;
