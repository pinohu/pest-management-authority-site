jQuery(document).ready(function ($) {
  ...
  $(".listing_submit_btn").on("click", function () {
    ...
    var error_messege =
      '<span class="atpp_required atbdp_make_str_red"><i class="fa fa-exclamation-triangle"></i> ' +
      plan_validator.crossLimit +
      "</span>";
  ...
  $("body").on("submit", "#atpp-change-plan-form", function (e) {
    ...
    var data = {
      ...
      order_id: active_elm.length
        ? active_elm.attr("data-value")
        : $("input[name='new_plan']:checked").attr("data-order_id"),
    ...
    $.post(
      ...
      function (response) {
        ...
        if (response.validation_error) {
          $("#directorist-claim-warning-notification")
            .addClass("text-warning")
            .html(response.validation_error);
        } else {
  ...
  $("body").on("click", ".dpp-order-select-dropdown ul li a", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $(
          ".dpp-selected-order-listing-type .atbdp_make_str_green, .dpp-selected-order-listing-type .atbdp_make_str_red",
        ).addClass("dpp-selected-order-listing-type--highlight");
        $(".atbd_modal_btn").removeClass("directorist-pricing-disabled");
        $(".atbd_modal_btn").removeAttr("disabled");
        setTimeout(() => {
          $(
            ".dpp-selected-order-listing-type .atbdp_make_str_green, .dpp-selected-order-listing-type .atbdp_make_str_red",
          ).removeClass("dpp-selected-order-listing-type--highlight");
        }, 2300);
      },
  ...
  $("body").on("click", ".directorist_required_login", function (e) {
    ...
    _this
      .closest(".directorist-pricing__action")
      .append(
        '<span class="dpp-email-warning">' +
          plan_validator.login_alert +
          "</span>",
      );
    return;
  });
  ...
  let guestInput =
    '<div style="margin-bottom: 10px;" class="directorist-form-group directorist_plan_guest_email"><input type="email" name="guest_customer_email" class="directorist-form-element guest_customer_email" placeholder="' +
    plan_validator.email_placeholder +
    '" required /></div>';
  ...
  $("body").on("keyup", ".guest_customer_email", function (e) {
    $(".directorist-pricing__action--btn").removeClass(
      "directorist_required_email",
    );
  });
